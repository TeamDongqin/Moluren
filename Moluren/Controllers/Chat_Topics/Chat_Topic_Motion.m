...
- (void)loadView
{
    ...
    [[TdTopic Instance] SetCurrentTopic:Topic_Motion];
}
...
-(void)SessionConnectedCallback
{
    ...
    UIImage *PowerButtonBgImage = [UIImage imageNamed:@"TopicMotion_PowerButton_On"];
...
-(void)receiveMessage{
    ...
    if(self.sharedConfig.isConnected && !self.sharedConfig.isReceivingMsg){
        if(self.sharedConfig.httpRequestTimeoutTimes>5 && self.sharedConfig.httpRequestTimeoutTimes%6==0){
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"陌路人" message:@"抱歉，亲，'旅社'内暂时没有遇到其他人，请回到大厅前往其他场景" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil];
            [alert show];
        }
...
- (void)sendZBPressed:(UIButton *)sender withText:(NSString *)text
{
    ...
    if(!self.sharedConfig.isConnected){
        [self sendDisconectedMessage:@"亲， 您已经不在'旅社'啦，'旅社'随时欢迎您回来！"];
    }else{
...
- (UIImage *)UserProfileMenuItem{
    return [UIImage imageNamed:@"TopicMotion_UserProfileMenuItem"];
}

- (UIImage *)DiceMenuItem{
    return [UIImage imageNamed:@"TopicMotion_DiceMenuItem"];
}

- (UIImage *)ChatHistoryMenuItem{
    return [UIImage imageNamed:@"TopicMotion_ChatHistoryMenuItem"];
}
