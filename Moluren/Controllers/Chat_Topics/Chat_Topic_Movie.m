...
@implementation Chat_Topic_Movie
...
//    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"影院" message:@"确定离开 '影院' 吗?" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
//    [alert show];
...
-(void)SessionConnectedCallback
{
    ...
    UIImage *PowerButtonBgImage = [UIImage imageNamed:@"TopicMovie_PowerButton_On"];
...
-(void)receiveMessage{
    ...
    if(self.sharedConfig.isConnected && !self.sharedConfig.isReceivingMsg){
        if(self.sharedConfig.httpRequestTimeoutTimes>5 && self.sharedConfig.httpRequestTimeoutTimes%6==0){
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"陌路人" message:@"抱歉，亲，'影院'内暂时没有遇到其他人，请回到大厅前往其他场景" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil];
            [alert show];
        }
...
- (void)sendZBPressed:(UIButton *)sender withText:(NSString *)text
{
    ...
    if(!self.sharedConfig.isConnected){
        [self sendDisconectedMessage:@"亲， 您已经不在'影院'啦，'影院'随时欢迎您回来！"];
    }else{
