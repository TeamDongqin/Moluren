...
-(void)receiveMessage{
    ...
    if(self.sharedConfig.isConnected && !self.sharedConfig.isReceivingMsg){
        if(self.sharedConfig.httpRequestTimeoutTimes>5 && self.sharedConfig.httpRequestTimeoutTimes%6==0){
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"陌路人" message:@"抱歉，亲，'步行街'内暂时没有联系其他人，请回到大厅前往其他场景" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil];
            [alert show];
        }
